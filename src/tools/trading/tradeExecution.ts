...
export const closePositionTool = createTool({
	...
	execute: async ({ symbol, percentage }) => {
		...
		try {
			...
			logger.info(
				`平仓 ${symbol} ${side === "long" ? "做多" : "做空"} ${closeSize}张 (入场: ${entryPrice.toFixed(2)}, 当前: ${currentPrice.toFixed(2)})`,
			);
			...
			logger.info(
				`  开仓价: ${entryPrice.toFixed(4)}, 平仓价: ${actualExitPrice.toFixed(4)}, 数量: ${actualCloseSize}张`,
			);
			logger.info(
