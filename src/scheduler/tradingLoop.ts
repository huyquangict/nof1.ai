...
async function collectMarketData() {
	...
	for (const symbol of SYMBOLS) {
		try {
			...
			const candles15m = await gateClient.getFuturesCandles(
				contract,
				"15m",
				96,
			);
			const candles30m = await gateClient.getFuturesCandles(
				contract,
				"30m",
				90,
			);
			...
			const openInterest = { latest: 0, average: 0 };
	...
	for (let i = 0; i < closes.length; i++) {
		...
		ema20Series.push(
			historicalPrices.length >= 20
				? calcEMA(historicalPrices, 20)
				: historicalPrices[historicalPrices.length - 1],
		);

		// MACD - 需要至少26个数据点
		macdSeries.push(
			historicalPrices.length >= 26 ? calcMACD(historicalPrices) : 0,
		);

		// RSI7 - 需要至少8个数据点
		rsi7Series.push(
			historicalPrices.length >= 8 ? calcRSI(historicalPrices, 7) : 50,
		);

		// RSI14 - 需要至少15个数据点
		rsi14Series.push(
			historicalPrices.length >= 15 ? calcRSI(historicalPrices, 14) : 50,
		);
	}
...
function calculateLongerTermContext(candles: any[]) {
	...
	const closes = candles
		...
		.filter((n) => Number.isFinite(n));
	const highs = candles
		.map((c) => Number.parseFloat(c.h || "0"))
		.filter((n) => Number.isFinite(n));
	const lows = candles
		.map((c) => Number.parseFloat(c.l || "0"))
		.filter((n) => Number.isFinite(n));
	const volumes = candles
		.map((c) => Number.parseFloat(c.v || "0"))
		.filter((n) => Number.isFinite(n));
...
async function syncPositionsFromGate(cachedPositions?: any[]) {
	...
	try {
		...
		const dbResult = await dbClient.execute(
			"SELECT symbol, sl_order_id, tp_order_id, stop_loss, profit_target, entry_order_id, opened_at FROM positions",
		);
		...
		if (activeGatePositions.length === 0 && dbResult.rows.length > 0) {
			logger.warn(
				`⚠️  Gate.io 返回0个持仓，但数据库有 ${dbResult.rows.length} 个持仓，可能是 API 延迟，跳过同步`,
			);
			return;
		}
...
async function getPositions(cachedGatePositions?: any[]) {
	...
	try {
		// 如果提供了缓存数据，使用缓存；否则重新获取
		const gatePositions =
			cachedGatePositions || (await gateClient.getPositions());
	...
	try {
		...
		for (const closeTrade of result.rows) {
			...
			if (pnlDiff > 0.5 || feeDiff > 0.1) {
				...
				logger.warn(
					`  盈亏: ${recordedPnl.toFixed(2)} → ${correctPnl.toFixed(2)} USDT (差异: ${pnlDiff.toFixed(2)})`,
				);
...
async function checkAccountThresholds(accountInfo: any): Promise<boolean> {
	...
	if (totalBalance <= accountRiskConfig.stopLossUsdt) {
		logger.error(
			`触发止损线！余额: ${totalBalance.toFixed(2)} USDT <= ${accountRiskConfig.stopLossUsdt} USDT`,
		);
	...
	if (totalBalance >= accountRiskConfig.takeProfitUsdt) {
		logger.warn(
			`触发止盈线！余额: ${totalBalance.toFixed(2)} USDT >= ${accountRiskConfig.takeProfitUsdt} USDT`,
		);
		await closeAllPositions(
...
async function executeTradingDecision() {
	...
	try {
		...
		for (const pos of positions) {
			...
			// 根据杠杆倍数确定止损线
			const leverageMid = Math.floor(
				(params.leverageMin + params.leverageMax) / 2,
			);
			const leverageHigh = Math.floor(
				params.leverageMin + (params.leverageMax - params.leverageMin) * 0.75,
			);
			...
			logger.info(
				`${symbol} 止损检查: 策略=${strategy}, 杠杆=${leverage}x, 止损线=${stopLossPercent}%, 当前盈亏=${pnlPercent.toFixed(2)}%`,
			);
			...
			if (!shouldClose) {
				...
				// 如果当前盈亏低于移动止损线
				if (
					pnlPercent < trailingStopPercent &&
					trailingStopPercent > stopLossPercent
				) {
					shouldClose = true;
			...
			if (shouldClose) {
				...
				try {
					...
					for (let retry = 0; retry < 5; retry++) {
						...
						try {
							const orderStatus = await gateClient.getOrder(
								order.id?.toString() || "",
							);
							...
							if (orderStatus.status === "finished") {
								...
								logger.info(
									`平仓成交: 价格=${actualExitPrice}, 数量=${actualQuantity}, 盈亏=${pnl.toFixed(2)} USDT`,
								);
								break;
							}
					...
					try {
						...
						logger.info(
							`  开仓价: ${pos.entry_price.toFixed(4)}, 平仓价: ${finalPrice.toFixed(4)}, 数量: ${actualQuantity}张`,
						);
						logger.info(
							`  净盈亏: ${pnl.toFixed(2)} USDT, 手续费: ${totalFee.toFixed(4)} USDT`,
						);
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
						...
						logger.info(
							`✅ 已记录强制平仓交易到数据库: ${symbol}, 盈亏=${pnl.toFixed(2)} USDT, 原因=${closeReason}`,
						);
					} catch (dbError: any) {
		...
		if (!dataValid) {
			...
			logger.error(
				`市场数据: ${Object.keys(marketData).length}, 账户: ${accountInfo?.totalBalance}, 持仓: ${positions.length}`,
			);
			return;
		}
		...
		try {
			...
			if (typeof response === "string") {
				...
				if (!decisionText) {
					decisionText =
						(response as any).text || (response as any).message || "";
				}
			...
			logger.info("=".repeat(80));
			logger.info(
				`账户: ${updatedAccountInfo.totalBalance.toFixed(2)} USDT (可用: ${updatedAccountInfo.availableBalance.toFixed(2)}, 收益率: ${updatedAccountInfo.returnPercent.toFixed(2)}%)`,
			);
			...
			if (updatedPositions.length === 0) {
				...
				updatedPositions.forEach((pos: any) => {
					...
					logger.info(
						`  ${pos.symbol} ${pos.side === "long" ? "做多" : "做空"} ${pos.quantity}张 (入场: ${pos.entry_price.toFixed(2)}, 当前: ${pos.current_price.toFixed(2)}, 盈亏: ${pos.unrealized_pnl >= 0 ? "+" : ""}${pos.unrealized_pnl.toFixed(2)} USDT / ${pnlPercent >= 0 ? "+" : ""}${pnlPercent.toFixed(2)}%)`,
					);
				});
			}

			logger.info(
				`未实现盈亏: ${finalUnrealizedPnL >= 0 ? "+" : ""}${finalUnrealizedPnL.toFixed(2)} USDT`,
			);
			logger.info("=".repeat(80) + "\n");
		} catch (agentError) {
