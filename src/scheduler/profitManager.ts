...
export async function runProfitManager(logger: Logger): Promise<void> {
  try {
    ...
    // Step 1.5: Update feedback for trading reflections (AI Learning)
    await updateReflectionFeedback(logger);
...
/**
 * Update feedback for trading reflections (AI Learning System)
 *
 * Fetches reflections created 10-60 minutes ago that don't have feedback yet,
 * calculates prediction accuracy and feedback scores, and updates the database.
 */
async function updateReflectionFeedback(logger: Logger): Promise<void> {
  try {
    ...
    if (learningEnabled.rows.length === 0 || learningEnabled.rows[0].value !== '1') {
      return; // Learning disabled, skip feedback updates
    }
    ...
    if (reflectionsResult.rows.length === 0) {
      return; // No reflections to update
    }
